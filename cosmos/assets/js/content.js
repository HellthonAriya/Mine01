/* ===========================================================================
   ایستگاهِ فضاییِ دمسا · بارگذارِ محتوا (قالب ۳ — کیهان)
   محتوا را از api/content (یا content.json) می‌گیرد، تمِ هوشمند را اعمال
   می‌کند، متنِ بخش‌ها را می‌نشاند و دادهٔ منو (سیاره‌ها/آیتم‌ها) را روی
   پیش‌فرض‌ها سوار می‌کند. با پنلِ ادمین از طریقِ postMessage پیش‌نمایشِ
   زنده دارد.
   =========================================================================== */
(function () {
  "use strict";
  ...
  const DEFAULT_THEME = { primary: "#7C5CFF", secondary: "#28E0C8" };
  ...
  function mergeData(content) {
    ...
    if (Array.isArray(d.planets) && d.planets.length) m.PLANETS = d.planets;
  ...
  function listenPreview() {
    window.addEventListener("message", (e) => {
      ...
      applyText(c.text || {});
      // داده‌ها فقط با بازخوانیِ iframe بازسازی می‌شوند (ساختارِ صحنه‌ای سنگین است)
    });
